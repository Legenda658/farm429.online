...
set_ajax_cache_headers(true, 15);
function log_message($message) {
    $log_dir = dirname(__FILE__) . '/../logs';
    if (!is_dir($log_dir)) {
        mkdir($log_dir, 0755, true);
    }
    error_log("[" . date("Y-m-d H:i:s") . "] get-lamp-state.php: " . $message . "\n", 3, $log_dir . "/api_activity.log");
}
try {
    ...
    $stmt = $pdo->prepare("SELECT lamp_state FROM sensor_data WHERE user_id = ? ORDER BY id DESC LIMIT 1");
    $stmt->execute([$user['id']]);
    $sensorData = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($sensorData) {
        log_message("Получено состояние лампы из sensor_data: " . $sensorData['lamp_state']);
        $lamp_state = (int)$sensorData['lamp_state'];
        echo json_encode([
            'success' => true,
            'state' => $lamp_state
        ]);
        ...
        $stmt = $pdo->prepare("INSERT INTO sensor_data (id, user_id, temperature, humidity, co2, soil_moisture, light_level, pressure, curtains_state, lamp_state) 
                              VALUES (?, ?, 25.0, 60.0, 800, 50.0, 1000.0, 760.0, 0, 0)");
        $result = $stmt->execute([$newId, $user['id']]);
        if ($result) {
            log_message("Создана новая запись с ID: $newId и состоянием лампы: 0");
            echo json_encode([
                'success' => true,
                'state' => 0
            ]);
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Ошибка при создании записи с состоянием лампы'
            ]);
        }
    }
