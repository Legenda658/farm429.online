...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT id FROM lighting_schedule WHERE user_id = :user_id AND is_exception = 0 LIMIT 1");
    $checkStmt->execute([':user_id' => $user_id]);
    $existingRecord = $checkStmt->fetch(PDO::FETCH_ASSOC);
    if ($existingRecord) {
        $stmt = $pdo->prepare("UPDATE lighting_schedule SET 
        ...
        $stmt = $pdo->prepare("INSERT INTO lighting_schedule (user_id, required_hours, start_time, end_time, is_exception) 
    ...
    $stmt = $pdo->prepare("DELETE FROM lighting_schedule WHERE user_id = :user_id AND is_exception = 1");
    $stmt->execute([':user_id' => $user_id]);
    if (!empty($exceptions)) {
        $stmt = $pdo->prepare("INSERT INTO lighting_schedule (user_id, start_time, end_time, is_exception) 
    ...
    $stmt = $pdo->prepare("SELECT temperature, humidity, photo, photo_analysis, comment, co2_level
        FROM farm_status 
    ...
    if ($existingStatus) {
        ...
        $stmt->execute([
            ':user_id' => $user_id,
            ':light_level' => $requiredHours
        ]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO farm_status (user_id, temperature, humidity, light_level, photo, photo_analysis, comment, co2_level, created_at)
            VALUES (:user_id, :temperature, :humidity, :light_level, :photo, :photo_analysis, :comment, :co2_level, NOW())");
        $stmt->execute([
            ':user_id' => $user_id,
    ...
    $stmtEventLog = $pdo->prepare("INSERT INTO event_log (user_id, event_type, event_description) VALUES (:user_id, 'lighting', :description)");
    $stmtEventLog->execute([
        ':user_id' => $user_id,
        ':description' => 'Установлено новое значение времени работы лампы: ' . $requiredHours . ' часов'
    ]);
    $pdo->commit();
